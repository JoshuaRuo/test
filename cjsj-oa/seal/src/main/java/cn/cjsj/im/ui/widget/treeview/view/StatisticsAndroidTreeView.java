package cn.cjsj.im.ui.widget.treeview.view;
...
import cn.cjsj.im.R;
import cn.cjsj.im.ui.widget.treeview.holder.StatisticsSimpleViewHolder;
import cn.cjsj.im.ui.widget.treeview.model.StatisticsTreeNode;

/**
 * Created by Bogdan Melnychuk on 2/10/15.
 */
public class StatisticsAndroidTreeView {
    private static final String NODES_PATH_SEPARATOR = ";";

    protected StatisticsTreeNode mRoot;
    private Context mContext;
    private boolean applyForRoot;
    private int containerStyle = 0;
    private Class<? extends StatisticsTreeNode.BaseNodeViewHolder> defaultViewHolderClass = StatisticsSimpleViewHolder.class;
    private StatisticsTreeNode.TreeNodeClickListener nodeClickListener;
    private StatisticsTreeNode.TreeNodeLongClickListener nodeLongClickListener;
    private boolean mSelectionModeEnabled;
    private boolean mUseDefaultAnimation = false;
    private boolean use2dScroll = false;
    private boolean enableAutoToggle = true;

    public StatisticsAndroidTreeView(Context context) {
        mContext = context;
    }

    public void setRoot(StatisticsTreeNode mRoot) {
        this.mRoot = mRoot;
    }

    public StatisticsAndroidTreeView(Context context, StatisticsTreeNode root) {
        mRoot = root;
        mContext = context;
    }
    ...
    public void setDefaultViewHolder(Class<? extends StatisticsTreeNode.BaseNodeViewHolder> viewHolder) {
        defaultViewHolderClass = viewHolder;
    }

    public void setDefaultNodeClickListener(StatisticsTreeNode.TreeNodeClickListener listener) {
        nodeClickListener = listener;
    }

    public void setDefaultNodeLongClickListener(StatisticsTreeNode.TreeNodeLongClickListener listener) {
    ...
    public void collapseAll() {
        for (StatisticsTreeNode n : mRoot.getChildren()) {
            collapseNode(n, true);
        }
    }
    ...
    public View getView(int style) {
        ...
        mRoot.setViewHolder(new StatisticsTreeNode.BaseNodeViewHolder(mContext) {
            @Override
    ...
    private void expandLevel(StatisticsTreeNode node, int level) {
        if (node.getLevel() <= level) {
            expandNode(node, false);
        }
        for (StatisticsTreeNode n : node.getChildren()) {
            expandLevel(n, level);
        }
    }

    public void expandNode(StatisticsTreeNode node) {
        expandNode(node, false);
    }

    public void collapseNode(StatisticsTreeNode node) {
    ...
    private void restoreNodeState(StatisticsTreeNode node, Set<String> openNodes) {
        for (StatisticsTreeNode n : node.getChildren()) {
    ...
    private void getSaveState(StatisticsTreeNode root, StringBuilder sBuilder) {
        for (StatisticsTreeNode node : root.getChildren()) {
            if (node.isExpanded()) {
    ...
    public void toggleNode(StatisticsTreeNode node) {
        if (node.isExpanded()) {
            collapseNode(node, false);
        } else {
            expandNode(node, false);
    ...
    private void collapseNode(StatisticsTreeNode node, final boolean includeSubnodes) {
        node.setExpanded(false);
        StatisticsTreeNode.BaseNodeViewHolder nodeViewHolder = getViewHolderForNode(node);
    ...
    private void expandNode(final StatisticsTreeNode node, boolean includeSubnodes) {
        node.setExpanded(true);
        final StatisticsTreeNode.BaseNodeViewHolder parentViewHolder = getViewHolderForNode(node);
        parentViewHolder.getNodeItemsView().removeAllViews();


        parentViewHolder.toggle(true);

        for (final StatisticsTreeNode n : node.getChildren()) {
    ...
    private void addNode(ViewGroup container, final StatisticsTreeNode n) {
        final StatisticsTreeNode.BaseNodeViewHolder viewHolder = getViewHolderForNode(n);
    ...
    public void setSelectionModeEnabled(boolean selectionModeEnabled) {
        ...
        for (StatisticsTreeNode node : mRoot.getChildren()) {
    ...
    public <E> List<E> getSelectedValues(Class<E> clazz) {
        ...
        List<StatisticsTreeNode> selected = getSelected();
        for (StatisticsTreeNode n : selected) {
    ...
    private void toggleSelectionMode(StatisticsTreeNode parent, boolean mSelectionModeEnabled) {
        toogleSelectionForNode(parent, mSelectionModeEnabled);
        if (parent.isExpanded()) {
            for (StatisticsTreeNode node : parent.getChildren()) {
                toggleSelectionMode(node, mSelectionModeEnabled);
            }
        }
    }

    public List<StatisticsTreeNode> getSelected() {
        if (mSelectionModeEnabled) {
    ...
    private List<StatisticsTreeNode> getSelected(StatisticsTreeNode parent) {
        List<StatisticsTreeNode> result = new ArrayList<>();
        for (StatisticsTreeNode n : parent.getChildren()) {
            if (n.isSelected()) {
                result.add(n);
            }
    ...
    public void selectNode(StatisticsTreeNode node, boolean selected) {
        if (mSelectionModeEnabled) {
    ...
    private void selectNode(StatisticsTreeNode parent, boolean selected, boolean skipCollapsed) {
    ...
    private void toogleSelectionForNode(StatisticsTreeNode node, boolean makeSelectable) {
        StatisticsTreeNode.BaseNodeViewHolder holder = getViewHolderForNode(node);
        if (holder.isInitialized()) {
            getViewHolderForNode(node).toggleSelectionMode(makeSelectable);
        }
    }

    private StatisticsTreeNode.BaseNodeViewHolder getViewHolderForNode(StatisticsTreeNode node) {
        StatisticsTreeNode.BaseNodeViewHolder viewHolder = node.getViewHolder();
        if (viewHolder == null) {
            try {
                final Object object = defaultViewHolderClass.getConstructor(Context.class).newInstance(mContext);
                viewHolder = (StatisticsTreeNode.BaseNodeViewHolder) object;
    ...
    public void addNode(StatisticsTreeNode parent, final StatisticsTreeNode nodeToAdd) {
        parent.addChild(nodeToAdd);
        if (parent.isExpanded()) {
            final StatisticsTreeNode.BaseNodeViewHolder parentViewHolder = getViewHolderForNode(parent);
            addNode(parentViewHolder.getNodeItemsView(), nodeToAdd);
        }
    }

    public void removeNode(StatisticsTreeNode node) {
        if (node.getParent() != null) {
            StatisticsTreeNode parent = node.getParent();
