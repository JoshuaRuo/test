...
/**
 * 申请积分
 * Created by LuoYang on 2018/2/7.
 */

public class ApplyIntegralActivity extends BaseActivity implements View.OnClickListener {
    @Bind(R.id.apply_integral_webview)
    WebView mWebView;

    @Bind(R.id.apply_integral_progressbar)
    ...
    private static final String BASE_WEEKLY_URL = ConstantValue.WEB_BASE_URL + "html/integral/my_integral.html";
    private boolean mIsIntegral = false;
    private boolean isContact = false;
    private boolean isRank = false;//排行榜
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_apply_integral);
        ...
        mWebView.setWebViewClient(new WebViewClient() {
            ...
            public boolean shouldOverrideUrlLoading(WebView view, String url) {
                if (url.contains("protocol://android.integral")) {  //积分事项
//                    finish();
                    mIsIntegral = true;
                    return true;
                }else if(url.contains("apply_integral.html")){
                    ...
                    return false;
                }else if (url.contains("integral_detail.html")) {
                    mWebHistoryUtil.setUrl(url);
                    mHisIndex++;
                    ...
                    return true;
                } else if (url.contains("integralRank.html")) {
                    mHeadRightText.setVisibility(View.VISIBLE);
                    mHeadRightText.setText("筛选");
                  return false;
                } else if (url.contains("integral_list.html")){
                    mWebHistoryUtil.setUrl(url);
                    mHisIndex++;
                    ...
                    return false;
                }else if (url.contains("integral_rank_self.html")){
                    mWebHistoryUtil.setUrl(url);
                    mHisIndex++;
                    mHeadRightText.setVisibility(View.INVISIBLE);
                    return false;
        ...
        switch (v.getId()) {
            case R.id.btn_left:
                if (isContact || mIsIntegral) {
                    ...
                    if (mHisIndex != 0) {//当webview不是处于第一页面时，返回上一个页面
                        ...
                        mHisIndex--;
                        if (mWebView.getUrl().contains("integralRank.html")) {
                            mHeadRightText.setVisibility(View.VISIBLE);
                            mHeadRightText.setText("筛选");
                            isRank = true;
                        } else if (mWebView.getUrl().contains("my_integral.html")) {
                            mHeadRightText.setVisibility(View.VISIBLE);
                            mHeadRightText.setText("积分榜");
                            mIsApplyHistory = false;
                            mIsApplyApproval = false;
                            ...
                            mHeadRightText.setVisibility(View.VISIBLE);
                        } else if (mWebView.getUrl().contains("integral_detail.html")) {
                            mIsApplyApproval = true;
                            mIsApplyHistory = false;
                            mHeadRightText.setText("审批进度");
                            mHeadRightText.setVisibility(View.VISIBLE);
                        }else if (mWebView.getUrl().contains("integral_list.html")){
                            mHeadRightText.setVisibility(View.INVISIBLE);
                            mIsApplyApproval = false;
                            mIsApplyHistory = false;
                        }
            ...
            case R.id.text_right:
                if (mIsApplyApproval) {
                    ...
                    mWebView.loadUrl(ConstantValue.WEB_BASE_URL + "html/integral/integralRank.html" + "?token=" + mToken + "&actDefId=" + mActDefId);
                    mWebHistoryUtil.setUrl(ConstantValue.WEB_BASE_URL + "html/integral/integralRank.html" + "?token=" + mToken + "&actDefId=" + mActDefId);
                    mHisIndex++;
                    mHeadRightText.setVisibility(View.VISIBLE);
                    isRank = true;
                    mHeadLayout.setBackgroundResource(R.color.color_2393FF);
                    setTitle("积分榜");
                    mHeadRightText.setText("筛选");
                    mBackBtn.setCompoundDrawablesWithIntrinsicBounds(getResources().getDrawable(R.drawable.back_white),null,null,null);
                } else {
                    mWebView.loadUrl(ConstantValue.WEB_BASE_URL + "html/integral/filter.html" + "?token=" + mToken + "&actDefId=" + mActDefId);
                    mWebHistoryUtil.setUrl(ConstantValue.WEB_BASE_URL + "html/integral/filter.html" + "?token=" + mToken + "&actDefId=" + mActDefId);
                    mHisIndex++;
                    mHeadRightText.setVisibility(View.INVISIBLE);
                    isRank = false;
                }
    ...
    class MyChromeViewClient extends WebChromeClient {
        ...
        public void onCloseWindow(WebView window) {
            ApplyIntegralActivity.this.finish();
        ...
        public boolean onJsAlert(WebView view, String url, String message,
            ...
            new AlertDialog.Builder(ApplyIntegralActivity.this)
                    .setTitle("提示信息")
                    .setMessage(message)
    ...
    class Controller {
        ...
        Activity getActivity() {
            return ApplyIntegralActivity.this;
        }
    }
